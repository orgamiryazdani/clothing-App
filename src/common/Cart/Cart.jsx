        ...
        <div className="flex flex-col max-w-[1440px] h-auto">
          <div className="flex flex-col h-36 justify-evenly w-screen px-8">
            ...
            <div className="flex items-center">
              ...
              <p className="text-gray-400 ml-1 text-lg">({cart.length})</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center md:items-start justify-center w-full h-4/6 p-4">
            {cart.map((product) => {
              return (
                <div key={product.id} className="flex flex-col m-2 md:m-1">
              ...
              cart.length > 0 ?
                <div className="w-full max-w-7xl mt-5 h-2/6">
                  <div className="flex justify-between px-4">
                    <p className="text-gray-400">Total Price ({cart.length} items)</p>
                    <span className="font-bold flex items-center text-lg">
                      <RiMoneyDollarCircleFill className="mr-1" />
                      {total}
                    </span>
                  ...
                  <div className="flex justify-between mt-8 px-3">
                    ...
                    <div className="w-2/6 h-auto relative">
                      ...
                      <div className="w-full min-w-[320px] max-w-[414px] absolute mt-4 min-h-0 h-auto flex items-center justify-end flex-col overflow-hidden rounded-xl bg-gray-300">
                        ...
                        <div className={`bg-gray-400 cursor-pointer duration-200 ease-in-out ${dropDown ? "h-10" : "h-0"} overflow-hidden w-full flex items-center justify-center font-bold text-xl`}>print receipt</div>
                      </div>
                    </div>
                    <button onClick={() => PLaceOrder(cart)} className="w-4/6 flex items-center justify-center rounded-xl text-white h-14 fill-[#ffffff] bg-black ml-2">
                      Place order <GrLinkNext className="text-white fill-white" />
                    </button>
                  </div>
                </div>
                : <p>cart is empty !</p>
